...
func move_camera(vector):
	var move = Vector2(rand_range(-vector.x, vector.x), rand_range(-vector.y, vector.y))
	get_parent().get_node("Isa").get_node("Camera2D").offset = move
	

func screen_shake(shake_time, shake_power, shake_priority):
